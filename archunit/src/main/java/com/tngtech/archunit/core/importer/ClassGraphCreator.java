...
class ClassGraphCreator implements ImportContext {
    ...
    private void resolveAnnotations(String annotationName) {
        if (classes.isPresent(annotationName)) {
            return;
        }

        classes.ensurePresent(annotationName);
        for (JavaAnnotationBuilder metaAnnotation : importRecord.getAnnotationsFor(annotationName)) {
            String metaAnnotationName = metaAnnotation.getFullyQualifiedClassName();
